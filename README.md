...
export PACKAGES_PATH ?= $(THIS_PATH)/packages
export INSTALL_PATH ?= $(THIS_PATH)/$(PACKAGES_PATH)/vendor
...
In addition to general contribution notes (Next Section), the following steps can be followed for adding new packages for review via a PR.
...
3. Test the install, ensure that it downloads and runs as expected.
